#!/usr/bin/env python3
"""Claude Code status line for Waystone — retrieval metrics + session info.

Installed as the ``waystone-statusline`` console script and wired into
``~/.claude/settings.json`` by ``waystone configure``. Displays standard session
info (model, context %, cost) plus Waystone metrics:
...
Reads the Claude Code session JSON on stdin and prints one status line.
"""
