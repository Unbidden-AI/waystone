...
1. **Extract** — `engram extract` sends a transcript to an OpenAI-compatible LLM (`extractor.py`). The LLM returns JSON with nodes (facts) and edges (relations). Short LLM-assigned IDs (`n1`, `n2`) are replaced with `n_<uuid8>` via `assign_ids()`. Nodes and edges are merged into SQLite via `GraphStore.merge_extraction()`.
