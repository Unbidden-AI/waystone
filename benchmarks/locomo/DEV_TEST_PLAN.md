...
|--------|---------|---------|
| `waystone_dedup95` | 63.5% | 64.9% (999/999) ✓ |
| `waystone_default_topk100` | 71.6% | 70.8% (999/999) ✓ |
| `waystone_semantic_rerank_topk100` | **73.9%** | **75.6% (999/999)** ✓ |
...
|--------|-------------------|-------|
| `waystone_dedup95` | *(none — runs extraction)* | Base extraction; dedup threshold 0.95 |
| `waystone_default_topk100` | `waystone_dedup95` | Copies DB; no re-extraction |
| `waystone_semantic_rerank_topk100` | `waystone_dedup95` | Copies DB; no re-extraction |
...
- `waystone_dedup95` runs extraction once; the other two configs copy its DB
...
- Note: 10 workers was tried but RPD exhaustion (10K/day cap) caused 953/2,997 failures.
  Judge cache (`~/.cache/waystone_judge_cache.json`) means ~2,044 already-scored QAs
...
- Configs: **sequential** — run `waystone_dedup95` first (extraction), then
...
| Judge — `waystone_dedup95` (999 QA × ~3,850 tok) | ~3.85M input | ~$0.58 |
| Judge — `waystone_default_topk100` | ~3.85M input | ~$0.58 |
| Judge — `waystone_semantic_rerank_topk100` | ~3.85M input | ~$0.58 |
| **Total** | | **~$1.78** |
...
| Phase | Wall-clock |
|-------|-----------|
| Extraction (`waystone_dedup95`, 5 convs parallel) | ~3–4 min |
...
- [x] Extraction complete — `waystone_dedup95` DBs built for all 5 convs
...
PYTHONUNBUFFERED=1 python3.13 -m benchmarks.locomo.harness \
  ...
  --split dev \
  --configs waystone_semantic_rerank_topk100 \
  --llm-judge \
...
python -m benchmarks.locomo.harness \
  --dataset benchmarks/locomo/data/locomo10.json \
  --split dev \
  --configs waystone_dedup95 waystone_default_topk100 waystone_semantic_rerank_topk100 \
  --extraction-model gemini-2.5-flash-lite \
  --judge-model gpt-4o-mini \
  --conv-workers 5
```
